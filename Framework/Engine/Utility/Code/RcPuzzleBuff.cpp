...
CRcPuzzleBuff::CRcPuzzleBuff() : m_pPos(nullptr)
{
}

CRcPuzzleBuff::CRcPuzzleBuff(LPDIRECT3DDEVICE9 pGraphicDev) : CVIBuffer(pGraphicDev), m_pPos(nullptr)

{
}

CRcPuzzleBuff::CRcPuzzleBuff(const CRcPuzzleBuff& rhs)
	: CVIBuffer(rhs)
	, m_hFile(rhs.m_hFile)
	, m_fH(rhs.m_fH)
	, m_iH(rhs.m_iH)
...
CRcPuzzleBuff::~CRcPuzzleBuff()
{
}

HRESULT CRcPuzzleBuff::Ready_Buffer(const _ulong& dwCntX, const _ulong& dwCntZ, const _ulong& dwVtxItv)
{
	m_dwFVF = FVF_TEX;
	...
	for (_ulong i = 0; i < dwCntZ; ++i)
	{
		for (_ulong j = 0; j < dwCntX; ++j)
		{
			...
			pVertex[dwIndex].vTexUV = { _float(j),
										_float(i) };
		}
...
void CRcPuzzleBuff::Render_Buffer(void)
{
	CVIBuffer::Render_Buffer();

}

CRcPuzzleBuff* CRcPuzzleBuff::Create(LPDIRECT3DDEVICE9 pGraphicDev, const _ulong& dwCntX, const _ulong& dwCntZ, const _ulong& dwVtxItv)
{
	CRcPuzzleBuff* pInstance = new CRcPuzzleBuff(pGraphicDev);

	if (FAILED(pInstance->Ready_Buffer(dwCntX, dwCntZ, dwVtxItv)))
	{
		Safe_Release(pInstance);
		MSG_BOX("PuzzleBuff Create Failed");
		return nullptr;
	}
...
CComponent* CRcPuzzleBuff::Clone(void)
{
	return new CRcPuzzleBuff(*this);
}

void CRcPuzzleBuff::Free(void)
{
	if (false == m_bClone)
	{
